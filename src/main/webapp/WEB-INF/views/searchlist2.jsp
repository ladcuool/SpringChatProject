...
<%
	...
	if(session.getAttribute("id")!=null) {
		...
		Sel=(String)session.getAttribute("Sel");
	} else {
...
<body>
	...
	<div class="d-flex justify-content-center">
		...
		<tbody>
				...
				<!-- 처음 -->
					<nav class="d-flex justify-content-center align-items-center" aria-label="...">
  						<ul class="pagination">
  							...
  							<c:otherwise>
  								<li class="page-item">
						    		<a class="page-link" href="search2?page=1&strTitle=${strlist}&Sel=<%=Sel%>" tabindex="-1" aria-disabled="true">처음으로</a>
    							</li>
  							</c:otherwise>
  							</c:choose>
  							<!-- 이전 -->
  							...
  							<c:otherwise>
  								<li class="page-item">
						    		<a class="page-link" href="search2?page=${page.curPage-1}&strTitle=${strlist}&Sel=<%=Sel%>" tabindex="-1" aria-disabled="true">이전</a>
    							</li>
  							</c:otherwise>
  							</c:choose>
  							...
  							<c:forEach var="fEach" begin="${page.startPage}" end="${page.endPage}" step="1">
  								...
  								<c:otherwise>
  									<li class="page-item">
					    				<a class="page-link" href="search2?page=${fEach}&strTitle=${strlist}&Sel=<%=Sel%>">${fEach}</a>
						   			 </li>
  								</c:otherwise>
  								</c:choose>
  							</c:forEach>
					    	...
					    	<c:otherwise>
					    		<li class="page-item">
      								<a class="page-link" href="search2?page=${page.curPage+1}&Sel=<%=Sel%>&strTitle=${strlist}">다음</a>
					    		</li>	
					    	</c:otherwise>
					    	</c:choose>
					    	<!-- 끝 -->
					    	...
					    	<c:otherwise>
					    		<li class="page-item">
      								<a class="page-link" href="search2?page=${page.totalPage}&strTitle=${strlist}&Sel=<%=Sel%>">끝으로</a>
					    		</li>	
					    	</c:otherwise>
					    	</c:choose>
  						</ul>
			...
			<tr>
				<td colspan="5"><div class="row justify-content-end"><a class="btn btn-primary" href="write" type="button" style="max-width:100px;">글쓰기</a></div> </td>
			</tr>
		</tbody>
	</table>
	...
	<div class="d-flex justify-content-center" style="max-height:60px;">
		<form class="d-flex" action="<%=application.getContextPath()%>/search2">
