...
<body>
	...
	<div class="container" style="width:50%" id="Users">
		<div class="container">
			<form action="<%=application.getContextPath()%>/reply?bId=${content_view.BId}" method="post">
				<div class="form-group">
	...
	<!-- 댓글창 출력  -->
	<%
		ArrayList<replyDto> list=(ArrayList<replyDto>)request.getAttribute("Table_view");
	...
	%>
		<script>
			$(document).ready(function (){
				$("#Users").append("<div class='form-group' style='margin-top:25px;'>"+
						...
						"<textarea class='form-control' name='Content' id='Content' rows='3' disabled style='background-color:white;'>"+
						"<%=replydto.getReplyContent()%></textarea></div>"+
						"<div class='d-flex justify-content-end' style='margin-top:10px;'><a class='btn btn-outline-primary' style='margin-right:5px;' href='reply_view?replyGroup=<%=replydto.getReplyGroup()%>&bId=<%=replydto.getBId()%>'>답글</a>"+
						"<%if(replyId.equals(id)) {%><button class='btn btn-outline-primary' onclick='revisefunction(<%=replydto.getBId()%>,<%=replydto.getReplyGroup()%>,\"<%=replydto.getReplyContent()%>\")' style='margin-right:5px;'>수정</button><a class='btn btn-outline-primary' href='/reply_delete?bId=<%=replydto.getBId()%>&replyGroup=<%=replydto.getReplyGroup()%>'>삭제</a><%}%></div>");
			});
		</script>
	<%
		}
